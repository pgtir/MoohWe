...
function Layout({ linkType, heading, query, movie_id }) {
  //  Defining States
  const [page, setPage] = useState(1);
  const [pageSearch, setPageSearch] = useState(1);
  const [isSearchPage, setIsPageSearch] = useState(false);
  const [isDetailsPage, setIsPageDetails] = useState(false);
  ...
  const categoriesURL = `${SERVER_URL}/discover/movie?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=${
    category && category.id
  }`;
  const generesURL =
    "https://api.themoviedb.org/3/genre/movie/list?api_key=cee71b2be49038091e9a4d19e2f5343e&language=en-US";
  ...
  const fetchMovies = async () => {
    let response;
    if (category)
      response = await axios.get(`${searchKey ? searchURL : categoriesURL}`);
    else
      response = await axios.get(`${searchKey ? searchURL : recommendedURL}`);
